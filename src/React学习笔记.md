...
又遇到一次情况是，React的代码写到最后发现，一开始认为自己的思路没有问题，最后完成的时候发现自己有很多的优化空间。可能是最近写的代码太多，或者是写多了想法开始变得灵活，也不知道是React带来的还是JS本身带来的。


## 我该怎样控制组件的粒度

在Hooks没出现前，函数式组件又称无状态组件，因为他们不在状态(不能控制State)，后来Hooks来了 于是拆分组件变得非常容易。于是这产生了另一个思考："怎么控制粒度呢?" 这个过度灵活的设计，可能会带来无尽的粒度拆分。过于细化，组件之间的传值会变得极为恶心。不得不一层一层的去写`onChange`往上抛。而不去做组件拆分选择一把梭，又会堆出来大几百行代码，成为一个后续难以维护的面条，在此我想用一个简单的行数限制，300行，这不是什么高深测算出的结果，只是我前段开发的经验谈，你也可以当成我拍脑袋说了个数。

## Hooks会让你觉得奇怪吗?

我在使用hooks的时候，总是觉得函数组件的生命周期不够明朗。不过说来也是，函数式本来就是轻量的。如果去规定各种生命周期，相比也是得不尝失。

## 未来的前端可能不会写js

前些日子看到一个段子，有个前端在当面试官的时候，看到面试对象快速的创建了一个.vue文件，然后开始在methods里面写function，Vue不愧是渐进式，渐进的后来有人都忘记了JS，以后招聘的时候会不会说自己招“React工程师” “Angular工程师”? 
